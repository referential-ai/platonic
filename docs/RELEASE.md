...
release version is `0.2.1`, while `platonic-core` keeps its own public semver.
...
proof-first. Desktop packages are not Platonic 0.2.1 release artifacts.
...
```text
platonic 0.2.1 (<40-character commit>, <YYYY-MM-DD>)
```
...
`release/platonic-0.2.1` from the accepted `develop` commit and limit it to release
...
Tag the exact accepted `main` merge as `platonic-v0.2.1`. The historical
...
| --- | --- | --- |
| Linux x86-64 | `x86_64-unknown-linux-gnu` | `platonic-0.2.1-linux-x86_64.tar.gz` |
| macOS Apple silicon | `aarch64-apple-darwin` | `platonic-0.2.1-macos-arm64.tar.gz` |
...
```bash
# Linux x86-64
sha256sum --check platonic-0.2.1-linux-x86_64.sha256

# macOS Apple silicon
shasum -a 256 --check platonic-0.2.1-macos-arm64.sha256
```
...
`release_tag=platonic-v0.2.1`; the workflow validates the tag against `main`,
