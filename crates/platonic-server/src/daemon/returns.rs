...
pub(super) fn reconcile_run(runtime: &DaemonRuntime, run_id: &str) -> AppResult<()> {
    ...
    store.settle_thread_run_deliveries(
        run_id,
        context_built_at.is_some() && run.status != RunStateName::Interrupted,
...
mod tests {
    ...
    #[test]
    fn interrupted_child_recovery_reuses_the_committed_parent_follow_up() {
        let (root, runtime, home, child, _, _) = child_return_test_runtime();
        let parent_run = admission(
            &runtime,
            "run-follow-up-parent",
            &home,
            "turn-follow-up-parent",
        );
        runtime
            .paths
            .server_store()
            .unwrap()
            .admit_thread_run_and_reserve(&parent_run, &[], &[])
            .unwrap();
        let answer = persist_parent_tool_answer(
            &runtime,
            &home,
            &parent_run.thread_turn_id,
            &parent_run.run_id,
            &ToolCallId::new("call-follow-up").unwrap(),
            ParentAnswerToolInput {
                child_thread_id: child.clone(),
                kind: ParentAnswerToolKind::FollowUp,
                payload: "continue from this exact committed payload".into(),
            },
            100,
        )
        .unwrap();
        let ParentAnswerToolOutput::Delivered { message_id, .. } = answer else {
            panic!("parent follow-up is delivered");
        };
        discard_run_admission(&runtime, &parent_run.run_id).unwrap();

        let interrupted_run_id = "run-follow-up-interrupted";
        let interrupted_turn_id = "turn-follow-up-interrupted";
        let (mut interrupted, _recorder, interrupted_identity) = prepared(
            &runtime,
            &child,
            interrupted_run_id,
            &root.path().join("follow-up-interrupted.jsonl"),
        );
        admit_spawn_edge_context(
            &runtime,
            &mut interrupted,
        ...
        .unwrap();
        assert!(context_text(&interrupted).contains("continue from this exact committed payload"));
        append_run_prefix(&runtime, &child, interrupted_run_id, &[], None);
        let interrupted_run = RunId::new(interrupted_run_id).unwrap();
        let mut ledger =
            SqliteLedger::open_or_create_default(&runtime.paths.default_ledger()).unwrap();
        ledger
            .append(
                ...
                &RecordedEvent {
                    ...
                    event: HarnessEvent::ContextBuilt {
                        run_id: interrupted_run.clone(),
                        turn_id: TurnId::new(interrupted_turn_id).unwrap(),
                        context: ContextPack {
                            token_budget: 1,
                            fragments: Vec::new(),
                        },
        ...
        let released = restarted
            ...
            .server_store()
            .unwrap()
            .available_parent_answers(&child, "unused")
            .unwrap();
        assert_eq!(released.len(), 1);
        assert_eq!(released[0].message_id, message_id);
        assert_eq!(released[0].state, DeliveryState::Available);
        assert_eq!(released[0].source_run_id, parent_run.run_id);

        let recovered_run_id = "run-follow-up-recovered";
        let recovered_turn_id = "turn-follow-up-recovered";
        let (mut recovered, _recorder, recovered_identity) = prepared(
            &restarted,
            &child,
            recovered_run_id,
            &root.path().join("follow-up-recovered.jsonl"),
        );
        admit_spawn_edge_context(
            &restarted,
            &mut recovered,
        ...
        .unwrap();
        assert_eq!(
            context_text(&recovered)
                .matches("continue from this exact committed payload")
                .count(),
            1
        );
        let reserved = restarted
            ...
            .server_store()
            .unwrap()
            .available_parent_answers(&child, recovered_run_id)
            .unwrap();
        assert_eq!(reserved.len(), 1);
        assert_eq!(reserved[0].message_id, message_id);
        assert_eq!(reserved[0].state, DeliveryState::Reserved);
        assert_eq!(
            reserved[0].reserved_by_run_id.as_deref(),
        ...
        append_run_prefix(
            ...
            recovered_run_id,
            &[],
            Some(Ok("continued from committed follow-up")),
        );
        reconcile_run(&restarted, recovered_run_id).unwrap();
        let consumed = restarted
            .paths
            .server_store()
            .unwrap()
