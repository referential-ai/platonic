...
| Provider rejects the model or endpoint | Use `/status` or `platonic status` to read `provider_kind`, `requested_alias`, and the served model when reported. Compare the resolved `kind`, `model`, and trusted `base_url`. | Correct the [provider configuration](../../../reference/configuration/#provider-shapes). Start a new durable thread when changing its requested model. |
