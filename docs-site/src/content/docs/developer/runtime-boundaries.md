...
1. **A client attaches.** For a one-shot question, Plato Agent ensures the host daemon, opens a local connection, completes `hello`, and calls `run.start` or `message.append`; it then polls `events.stream`. In TUI mode it selects or creates a profile, resolves or approves `profile.open`, and attaches the resulting home before using `thread.send` and `thread.events`. Neither client path drives the run itself. Read [`plato-agent/src/run.rs`](https://github.com/referential-ai/platonic/blob/develop/crates/plato-agent/src/run.rs) and [`plato-agent/src/profile.rs`](https://github.com/referential-ai/platonic/blob/develop/crates/plato-agent/src/profile.rs).
...
A workspace identifies the durable scope. A profile owns one home and supplies versioned defaults. A thread freezes the authority allowed for an ongoing conversation; same-profile children can only narrow it. Each new profile turn selects the current immutable content revision, and each admitted run executes within the recorded projection and produces one ordered event history. This chain is server-owned; clients select and observe it through protocol methods rather than recreating it. The exact ownership rule is in the [repository guide](https://github.com/referential-ai/platonic/blob/develop/AGENTS.md#repo-boundary), and the complete thread record is proven by the [`thread_authority_persists_profile_classification_and_is_immutable_after_restart` test](https://github.com/referential-ai/platonic/blob/develop/crates/platonic-server/src/server_store/queries.rs).
