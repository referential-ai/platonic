...
On supported Linux hosts, the server applies Landlock to the run child using the thread's writable authority and scratch path. If confinement is unavailable, the durable fact is `none`; a server configured to require confinement rejects the run instead. This is an execution boundary, not a client promise. Read the [server overview](https://github.com/referential-ai/platonic/blob/develop/README.md#server) and [`confinement.rs`](https://github.com/referential-ai/platonic/blob/develop/crates/platonic-server/src/confinement.rs). Setup and operational policy belong to [#548](https://github.com/referential-ai/platonic/issues/548).
